...
export default function CreacionConceptosPage() {
  ...
  const fetchData = async (page = 1, rif?: string, forma?: string) => {
    try {
      ...
      setTotalPlanillas(data.pagination?.total || 0);
  ...
  // Filtrar códigos presupuestarios cuando cambie la búsqueda
  useEffect(() => {
    if (codigosPresupuestarios.length > 0) {
  ...
  const handleProcesarVinculaciones = async () => {
    if (vinculaciones.length === 0) {
      toast.error('Debe seleccionar al menos una vinculación');
      return;
    }
    ...
    try {
      ...
      setProgressLog(prev => [
        ...prev, 
        `🚀 Iniciando procesamiento de ${vinculaciones.length} vinculación(es)...`,
        `📋 Vinculaciones a procesar:`,
        ...vinculaciones.map(v => `   • ${v.nombre_forma} → ${v.codigo_presupuestario} - ${v.designacion_presupuestario}`),
      ...
      if (!response.ok) throw new Error('Error procesando vinculaciones');
      
      setProgressLog(prev => [...prev, `📡 Respuesta recibida del servidor...`]);
      ...
      const result: ResultadoCreacion = responseData.data;
      
      // Agregar logs de resultado
      setProgressLog(prev => [
        ...prev,
        `✅ Procesamiento completado exitosamente`,
        `📊 Resultados finales:`,
        `   • Planillas procesadas: ${result.planillas_procesadas.toLocaleString()}`,
        `   • Conceptos creados: ${result.conceptos_creados.toLocaleString()}`,
        `   • Planillas validadas: ${result.planillas_validadas.toLocaleString()}`,
        result.errores && result.errores.length > 0 ? `   • Errores: ${result.errores.length}` : `   • Errores: 0`,
        `🔄 Actualizando vistas materializadas...`,
        `🔄 Recargando datos del módulo...`
      ]);
      
      // Mostrar mensaje de éxito con detalles
      let mensaje = `Proceso completado: ${result.conceptos_creados} conceptos creados, ${result.planillas_validadas} planillas validadas`;
      
      if (result.errores && result.errores.length > 0) {
        mensaje += ` (${result.errores.length} errores)`;
        console.warn('Errores durante el proceso:', result.errores);
      }
      
      toast.success(mensaje);
      
      // Recargar datos
      fetchData(currentPage, filtroRif, filtroForma);
      setVinculaciones([]);
      
      setProgressLog(prev => [...prev, `✅ Proceso completado exitosamente!`]);
      ...
      console.error('Error:', error);
      setProgressLog(prev => [...prev, `❌ Error: ${error instanceof Error ? error.message : 'Error desconocido'}`]);
      toast.error('Error procesando vinculaciones');
      
      // Cerrar modal después de 5 segundos en caso de error
      setTimeout(() => {
        setShowProgressModal(false);
        setProgressLog([]);
      }, 5000);
  ...
  const verPlanillasForma = async (forma: FormaAgrupada) => {
    try {
      ...
      const response = await fetch(`/api/creacion-conceptos/planillas-forma?codigo_forma=${forma.codigo_forma}&limit=20`);
      if (!response.ok) throw new Error('Error cargando planillas');
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Total Planillas Sin Conceptos</CardTitle>
            <IconFileInvoice className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalPlanillas.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              Planillas pendientes de vinculación
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatNumber(totalMonto)}</div>
            <p className="text-xs text-muted-foreground">
              Monto total de planillas sin conceptos
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Formas Diferentes</CardTitle>
            <IconAlertTriangle className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formasAgrupadas.length}</div>
            <p className="text-xs text-muted-foreground">
              Tipos de formas sin conceptos
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {formasAgrupadas.map((forma) => (
                <div key={forma.codigo_forma} className="p-4 border rounded-lg">
                  <div className="mb-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h4 className="font-semibold text-sm">{forma.nombre_forma}</h4>
                      ...
                      <Button
                        ...
                        className="ml-2 h-8 w-8 p-0 hover:bg-blue-50 hover:text-blue-600 transition-all duration-200"
                        title="Ver planillas"
                      >
                  ...
                  <TableBody>
                    {planillas.map((planilla) => (
                      <TableRow key={planilla.planilla_id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">{planilla.codigo_forma}</Badge>
                        </TableCell>
                        <TableCell>{planilla.nombre_forma}</TableCell>
                        <TableCell>
                          {planilla.fecha_trans ? new Date(planilla.fecha_trans).toLocaleDateString('es-VE') : 'N/A'}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        fetchData(Math.max(1, currentPage - 1), filtroRif, filtroForma);
                      }}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        fetchData(Math.min(totalPages, currentPage + 1), filtroRif, filtroForma);
                      }}
      ...
      {showProgressModal && (
       ...
       {showPlanillasModal && (
         <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-in fade-in duration-200">
           <div className="bg-white rounded-lg p-6 w-full max-w-4xl mx-4 max-h-[90vh] overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
             <div className="flex items-center justify-between mb-4">
               <div>
