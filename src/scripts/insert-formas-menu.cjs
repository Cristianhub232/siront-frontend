...
async function insertFormasMenu() {
  try {
    await sequelize.authenticate();
    console.log('✅ Conexión a la base de datos establecida correctamente.');

    // Insertar el menú de consulta de formas
    const menuId = uuidv4();
    ...
    if (result && result.length > 0) {
      console.log('✅ Menú de consulta de formas insertado correctamente:');
      console.log('ID:', result[0].id);
      console.log('Key:', result[0].key);
      console.log('Label:', result[0].label);
    } else {
      console.log('ℹ️ El menú de consulta de formas ya existe o no se pudo insertar.');
    }
